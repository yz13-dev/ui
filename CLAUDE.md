...
`registry/` is the single source of truth for every UI component (published and distributable via `registry.json` / `shadcn add`). There is no `components/ui/*` shim layer — this repo imports components directly from `@/registry/components/ui/<name>`, including registry components importing each other (e.g. `sidebar.tsx` importing `Button` uses `@/registry/components/ui/button`, not a `components/ui` alias). `components/ui/<name>.tsx` only exists as the *install target* declared in `registry.json` for downstream consumers running `shadcn add` — it is never a real path in this repo.

When adding a new UI component:

1. Create it in `registry/components/ui/<name>.tsx`, importing any other registry component via `@/registry/components/ui/<other-name>`.
2. Add an entry to `registry.json` (`type: "registry:block"`, `files` pointing at `registry/components/ui/<name>.tsx` → target `components/ui/<name>.tsx`, which is only meaningful for a consumer's install location). Declare `dependencies` only for packages a standard shadcn install wouldn't already have — notably `@base-ui/react` (this project uses it instead of Radix) and any component-specific library (`cmdk`, `vaul`, `recharts`, etc.). Declare `registryDependencies` for any other registry item it imports.

A component that's genuinely docs-site-only (not meant for external distribution) can skip the registry and live directly in `components/` instead.
